    ...
    if (erros.length > 0) {
        ...
        Usuario.findOne({email: req.body.email}).then((usuario) => {
            if (usuario) {
                ...
                brypt.genSalt(10, (erro, salt) => {
                    brypt.hash(novoUsuario.senha, salt, (erro, hash) => {
                        if (erro) {
                            req.flash("error_msg", "Houve um erro ao tentar registrar o usuario");
                            res.redirect("/");
                        }
                        ...
                        novoUsuario.save().then(() => {
                            ...
                            req.flash("error_msg", "Houve um erro ao tentar registrar o usuario. Tente novamente");
                            res.redirect("/usuarios/registro");
                        });
                    });
                });
